...
# ==========================================
# GENERAL CATEGORIES EXECUTION
# ==========================================

def run_agent_general_categories():
    print("="*80, flush=True)
    print(" BIOTECH CATALYST INTELLIGENCE AGENT (GENERAL CATEGORIES MASTER REPORT)", flush=True)
    print("="*80, flush=True)

    master_html_sections = []
    total_articles_ingested = 0

    now_str = datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')
    master_html_sections.append("<h2>Biotech Catalyst Intelligence Master Report</h2>")
    master_html_sections.append(f"<p><b>Scope:</b> Top-Level & General Indications<br><b>Generated:</b> {now_str}</p>")
    master_html_sections.append("<hr>")

    # Loop through every primary category
    for cat_name, cat_content in PRIMARY_CATEGORIES.items():
        if cat_name in SUBCATEGORY_PARENTS:
            # Pick the "General" subcategory (e.g., "General Oncology", "General Neuroscience")
            general_key = next((k for k in cat_content.keys() if "General" in k), list(cat_content.keys())[0])
            queries = cat_content[general_key]
            label = f"{cat_name} - {general_key}"
        else:
            # Standalone category (e.g., AI Drug Discovery)
            queries = cat_content
            label = cat_name

        print(f"\n[*] Processing Category: {label}", flush=True)

        # 1. Gather RSS Data
        news_items = gather_category_data(label, queries)
        total_articles_ingested += len(news_items)

        # 2. LLM Analysis
        try:
            report = analyze_catalysts_with_llm(label, news_items)
            section_html = build_blog_html(report, news_items)
            master_html_sections.append(section_html)
            master_html_sections.append("<br><br><hr><br><br>")
        except Exception as e:
            print(f"[!] Error analyzing {label}: {e}", flush=True)
            master_html_sections.append(f"<h3>{label}</h3><p><i>Analysis failed due to error: {e}</i></p><hr>")

    # Combine sections into one post
    final_post_html = "\n".join(master_html_sections)
    post_title = f"Master Biotech Catalyst Report: General Indications ({datetime.utcnow().strftime('%Y-%m-%d')})"

    print(f"\n[*] All general categories processed! Total articles: {total_articles_ingested}", flush=True)
    print("[*] Publishing Master Report to WordPress...", flush=True)

    try:
        published = publish_to_wordpress(post_title, final_post_html)
        print(f"[+] Master Post published LIVE successfully! URL: {published.get('URL')}", flush=True)
    except Exception as e:
        print(f"\n[!] Error publishing Master Report to WordPress: {e}", flush=True)

if __name__ == "__main__":
    run_agent_general_categories()
